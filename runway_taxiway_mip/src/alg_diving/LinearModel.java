...
public class LinearModel {
	...
	public void linearSolve(int mps, int cpuTime, List<ArcFlight> afList, Flight fixFlight){
		...
		try{
			...
			//fix arc flight from list
			for(ArcFlight af:afList){
				cplex.addEq(omega[af.getFlight().getId()][af.getArc().getId()], 1);
			}
			...
			//solve
			if(cplex.solve()){
				System.out.println("CPS ARC obj = "+String.format("%.1f",cplex.getObjValue()));
				
				Path path = new Path();
				Node sNode = nodeMap.get("o");
				path.getNodeList().add(sNode);
				
				boolean outLoop = false;
				System.out.println(fixFlight);
				while(!sNode.getCallsign().contains("d")){
					String sCallsign = sNode.getCallsign();
					for(Arc arc:sNode.getOutArcList()){	
						if(Math.abs(cplex.getValue(omega[fixFlight.getId()][arc.getId()])-1)<0.001){
							ArcFlight af = new ArcFlight();
							af.setFlight(fixFlight);
							af.setArc(arc);
							afList.add(af);
							sNode = arc.getEndNode();
							System.out.print(arc+" ");
							path.getNodeList().add(sNode);
							path.getArcList().add(arcMap.get(sCallsign+"_"+sNode.getCallsign()));
							break;
						}else if(cplex.getValue(omega[fixFlight.getId()][arc.getId()])>=0.5){
							ArcFlight af = new ArcFlight();
							af.setFlight(fixFlight);
							af.setArc(arc);
							afList.add(af);
							sNode = arc.getEndNode();
							outLoop = true;
							System.out.print(arc+" "+cplex.getValue(omega[fixFlight.getId()][arc.getId()]));
							path.getNodeList().add(sNode);
							path.getArcList().add(arcMap.get(sCallsign+"_"+sNode.getCallsign()));
							break;
						}
					...
					if(outLoop)break;
					if(sNode.getCallsign().contains("d")){
						System.out.println("Find A Complete Path!");
						fixFlight.setPath(path);   //dummy path, can instead build a path here
						path.setF(fixFlight);
					}
				}
