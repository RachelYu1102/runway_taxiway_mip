...
public class TaxiwayModel {
	...
	public double integerSolve(int mps, int cpuTime, List<SequencePair> spList){
		...
		try{
			...
			for(SequencePair sp:spList){
				Flight headF = sp.getHeadF();
				Flight tailF = sp.getTailF();
				if(headF.isArr()){
					cplex.addEq(y[headF.getId()][tailF.getId()][pseudoOriginId],1);
				}else{
					cplex.addEq(y[headF.getId()][tailF.getId()][pseudoDestinId],1);
				}
