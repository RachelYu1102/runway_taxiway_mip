...
public class FindPathModel {
	...
	private boolean runwayConflictCheck = false;
	private int mps = 3; //maximumPositionShifting
	
	public void integerSolve(int mps, int cpuTime, List<Flight> flightList, List<Flight> tempFix,
...
/*			//variable fixing by position and mps
			...
			//fix the path for tempFix
			for(Flight f:tempFix){
				Path path = f.getPath();
				for(int l=0;l<path.getArcList().size();l++){
					Arc arc = path.getArcList().get(l);
					cplex.addEq(omega[f.getId()][arc.getId()],1);  
				}	
			}
			...
			//solve
			if(cplex.solve()){
				System.out.println("Sub Group obj = "+String.format("%.1f",cplex.getObjValue()));
				...
				for(int i=0;i<flightSize;i++){
					...
					Path path = new Path();
					path.setF(flightList.get(i));
					flightList.get(i).setPath(path);
