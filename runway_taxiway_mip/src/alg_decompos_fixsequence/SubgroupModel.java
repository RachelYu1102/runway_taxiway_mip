		...
		try{
			...
			if(cplex.solve()){
				...
				for(int i=0;i<flightSize;i++){
					...
					flightList.get(i).setPath(route);
				...
				for(int i=0;i<flightList.size();i++){
					Flight headF = flightList.get(i);
					for(int j=0;j<flightList.size();j++){
						if(i==j)continue;
						Flight tailF = flightList.get(j);
						for(Node n:headF.getPath().commonNode(tailF.getPath())){
							//System.out.println(n+" "+cplex.getValue(y[headF.getId()][tailF.getId()][n.getId()]));
							if(Math.abs(cplex.getValue(y[headF.getId()][tailF.getId()][n.getId()])-1)<0.001){
