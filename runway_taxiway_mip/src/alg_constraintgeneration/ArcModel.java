...
public class ArcModel {
	...
	public double integerSolve(int mps, int cpuTime, List<ConstraintNode> consNodes, List<ConstraintArc> consArcs, List<ConstraintArc> consReArcs){
		...
		try{
			...
			for(ConstraintNode cn:consNodes){
				int i = cn.getF1().getId();
				int j = cn.getF2().getId();
				Node n = cn.getNode();
			...
			for(ConstraintArc ca:consArcs){
				int i = ca.getF1().getId();
				int j = ca.getF2().getId();
				Node eNode = ca.getArc().getEndNode();
				Node sNode = ca.getArc().getStartNode();
				Arc arc = ca.getArc();
				...
				//related node separation
				IloLinearNumExpr taxiNodeExpr = cplex.linearNumExpr();
				taxiNodeExpr.addTerm(1.0,t[i][eNode.getId()]);
				taxiNodeExpr.addTerm(-1.0,t[j][eNode.getId()]);
				taxiNodeExpr.addTerm(-bigM,x[i][eNode.getId()]);
				taxiNodeExpr.addTerm(-bigM,x[j][eNode.getId()]);
				taxiNodeExpr.addTerm(-bigM,y[j][i][eNode.getId()]);
				//cplex.addGe(taxiNodeExpr,traSep[j]-3*bigM);
				cplex.addGe(taxiNodeExpr,taxiSep-3*bigM);
				
				IloLinearNumExpr taxiNodeEpr2 = cplex.linearNumExpr();
				taxiNodeEpr2.addTerm(1.0,t[j][eNode.getId()]);
				taxiNodeEpr2.addTerm(-1.0,t[i][eNode.getId()]);
				taxiNodeEpr2.addTerm(-bigM,x[i][eNode.getId()]);
				taxiNodeEpr2.addTerm(-bigM,x[j][eNode.getId()]);
				taxiNodeEpr2.addTerm(-bigM,y[i][j][eNode.getId()]);
				//cplex.addGe(taxiNodeEpr2,traSep[i]-3*bigM);
				cplex.addGe(taxiNodeEpr2,taxiSep-3*bigM);
				
				IloLinearNumExpr orderPrecedenceExprE = cplex.linearNumExpr();
				orderPrecedenceExprE.addTerm(1.0,y[i][j][eNode.getId()]);
				orderPrecedenceExprE.addTerm(1.0,y[j][i][eNode.getId()]);
				cplex.addEq(orderPrecedenceExprE,1);
				
				//start node
				IloLinearNumExpr taxiNodeExprS = cplex.linearNumExpr();
				taxiNodeExprS.addTerm(1.0,t[i][sNode.getId()]);
				taxiNodeExprS.addTerm(-1.0,t[j][sNode.getId()]);
				taxiNodeExprS.addTerm(-bigM,x[i][sNode.getId()]);
				taxiNodeExprS.addTerm(-bigM,x[j][sNode.getId()]);
				taxiNodeExprS.addTerm(-bigM,y[j][i][sNode.getId()]);
				//cplex.addGe(taxiNodeExpr,traSep[j]-3*bigM);
				cplex.addGe(taxiNodeExprS,taxiSep-3*bigM);
				
				IloLinearNumExpr taxiNodeEpr2S = cplex.linearNumExpr();
				taxiNodeEpr2S.addTerm(1.0,t[j][sNode.getId()]);
				taxiNodeEpr2S.addTerm(-1.0,t[i][sNode.getId()]);
				taxiNodeEpr2S.addTerm(-bigM,x[i][sNode.getId()]);
				taxiNodeEpr2S.addTerm(-bigM,x[j][sNode.getId()]);
				taxiNodeEpr2S.addTerm(-bigM,y[i][j][sNode.getId()]);
				//cplex.addGe(taxiNodeEpr2,traSep[i]-3*bigM);
				cplex.addGe(taxiNodeEpr2S,taxiSep-3*bigM);
				
				IloLinearNumExpr orderPrecedenceExprS = cplex.linearNumExpr();
				orderPrecedenceExprS.addTerm(1.0,y[i][j][sNode.getId()]);
				orderPrecedenceExprS.addTerm(1.0,y[j][i][sNode.getId()]);
				cplex.addEq(orderPrecedenceExprS,1);
			}
			
			for(ConstraintArc ca:consReArcs){
			...
			//solve
			if(cplex.solve()){
				System.out.println("Constraint Feed obj = "+String.format("%.1f",cplex.getObjValue()));
				...
				for(int i=0;i<flightSize;i++){
					...
					while(!sNode.getCallsign().contains("d")){		
						System.out.print(sNode+"("+Math.round(cplex.getValue(t[i][sNode.getId()]))+")-");
						Node eNode = showMap.get(sNode).getEndNode();
						sNode = eNode;
						route.getNodeList().add(sNode);
						route.getTimeList().add((int)Math.round(cplex.getValue(t[i][sNode.getId()])));
					}
					System.out.println(sNode+"("+Math.round(cplex.getValue(t[i][sNode.getId()]))+")-");				
				...
				/*for(ConstraintArc ca:consReArcs){
					...
					Arc arc = ca.getArc();
					Arc reverseArc = arcMap.get(eNode.getCallsign()+"_"+sNode.getCallsign());
					System.out.println("yij e:"+cplex.getValue(y[i][j][eNode.getId()]));
					System.out.println("yij s:"+cplex.getValue(y[i][j][sNode.getId()]));
					System.out.println("yji e:"+cplex.getValue(y[j][i][eNode.getId()]));
					System.out.println("yji s:"+cplex.getValue(y[j][i][sNode.getId()]));
					System.out.println("w a:"+cplex.getValue(omega[i][arc.getId()]));
					System.out.println("w ra:"+cplex.getValue(omega[j][reverseArc.getId()]));					
				}*/
