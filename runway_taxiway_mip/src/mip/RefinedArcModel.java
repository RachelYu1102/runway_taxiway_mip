...
import entity.*;
import experiment.MyFile;
import ilog.concert.*;
import ilog.cplex.IloCplex;

public class RefinedArcModel {
	...
	private boolean gateConflictCheck = false;
	...
	public double integerSolve(int mps, int cpuTime){
		...
		try{
			...
			cplex.setParam(IloCplex.DoubleParam.EpGap, 0.01);   //set gap for termination
			//solve
			if(cplex.solve()){
				try {
					//String outputName = "result/output_schedule_comma.csv";
					String outputName = "result/output_schedule_dash.csv";
					...
					//show results	
					//ReadData.gap = cplex.getMIPRelativeGap();
					for(int i=0;i<flightSize;i++){
						...
						sb.append(flightList.get(i).getCallsign()+",");
						for(Arc arc:arcList){
						...
						while(!sNode.getCallsign().contains("d")){
							...
							sb.append(sNode+"("+Math.round(cplex.getValue(t[i][sNode.getId()]))+"),");
							Node eNode = showMap.get(sNode).getEndNode();	
							sNode = eNode;
						}
						System.out.println(sNode+"("+String.format("%.1f",cplex.getValue(t[i][sNode.getId()]))+"),");				
						sb.append(sNode+"("+(int)Math.round(cplex.getValue(t[i][sNode.getId()]))+")\n");
