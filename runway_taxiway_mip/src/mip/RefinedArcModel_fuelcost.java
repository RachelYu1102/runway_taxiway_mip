...
public class RefinedArcModel_fuelcost {
	...
	public double integerSolve(int mps, int cpuTime){
		...
		try{
			...
			for(int i=0;i<flightSize;i++){
				//objective.addTerm(1,t[i][dNode[i]]);	  //useless
				//objective.addTerm(1,t[i][pseudoDestinId]);  //basic
				if(flightList.get(i).isArr()){				//fuel cost
					objective.addTerm(1,t[i][pseudoDestinId]);
				}else{
					objective.addTerm(1,t[i][pseudoDestinId]);
					objective.addTerm(1,t[i][oNode[i]]);
				}
			}
