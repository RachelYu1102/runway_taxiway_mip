...
public class ArrSwapGateModel {
	...
	public void integerSolve(int mps, int cpuTime){
		...
		try{
			...
			for(int i=0;i<flightSize;i++){
				...
				if(f.isArr()){
					...
					IloLinearNumExpr destinNodeExpr = cplex.linearNumExpr(); 
					for(Node n:f.getCandidateDestinNodes()){
						destinNodeExpr.addTerm(1, x[i][n.getId()]);
					}
					cplex.addEq(destinNodeExpr,1);
					//originArc
					for(Arc a:psudoOArc){
			...
			//gate conflict -- arr & arr
			for(Arc a:psudoDArc){
				if(a.getStartNode().getCallsign().contains("e"))continue;
				IloLinearNumExpr arrGateConflictExpr = cplex.linearNumExpr(); 
				for(Flight f:arrList){
					arrGateConflictExpr.addTerm(1, omega[f.getId()][a.getId()]);
				}
				cplex.addEq(arrGateConflictExpr,1);
			}
			//gate conflict -- arr & dep
			if(gateConflictCheck){
				for(Flight arr:arrList){
					for(Flight dep:depList){						
						if(arr.getCandidateDestinNodes().contains(dep.getOriginNode())){
							IloLinearNumExpr gateConflictExpr = cplex.linearNumExpr();
							gateConflictExpr.addTerm(1, t[arr.getId()][oNode[dep.getId()]]);
							gateConflictExpr.addTerm(-1, t[dep.getId()][oNode[dep.getId()]]);
							gateConflictExpr.addTerm(-bigM, x[arr.getId()][oNode[dep.getId()]]);
							cplex.addGe(gateConflictExpr,pushT[dep.getId()]-bigM);
						}
					}
				}
			}
			...
			//variable fixing according to FCFS position
			for(Flight f:depList){
				for(Flight f2:depList){
					if(f.equals(f2))continue;
					if(f.getFCFSposition()-f2.getFCFSposition()>=2*mps){
						cplex.addEq(y[f2.getId()][f.getId()][pseudoDestinId],1);
					}
				}
			}
			for(Flight f:arrList){
				for(Flight f2:arrList){
					if(f.equals(f2))continue;
					if(f.getFCFSposition()-f2.getFCFSposition()>=2*mps){
						cplex.addEq(y[f2.getId()][f.getId()][pseudoOriginId],1);
					}
				}
			}
			//valid inequalities that enforce transitive 3-tuple (i,j,k)
			...
			//valid inequalities, earliest time on destination node
			
			
			//fixing variables: arrival in the same class
			...
			//cplex.addEq(omega[2][37], 1);
			//cplex.addEq(omega[4][37], 1);
			//cplex.addEq(omega[3][37], 1);
		/*	cplex.addEq(y[1][0][nodeMap.get("d").getId()], 1);
			cplex.addEq(y[0][2][nodeMap.get("d").getId()], 1);
			cplex.addEq(y[3][5][nodeMap.get("o").getId()], 1);
			cplex.addEq(y[5][4][nodeMap.get("o").getId()], 1);*/
			//cplex.addEq(t[2][nodeMap.get("n9").getId()], 480);
			//cplex.addEq(t[2][nodeMap.get("n8").getId()], 660);
			//cplex.addEq(t[4][nodeMap.get("n9").getId()], 505);
			//cplex.addEq(t[4][nodeMap.get("n8").getId()], 565);
			//cplex.addEq(t[3][nodeMap.get("n8").getId()], 360);
			...
			if(cplex.solve()){
				...
				for(int i=0;i<flightSize;i++){
					...
					System.out.print(flightList.get(i)+" t(dNode)"+ String.format("%.1f",cplex.getValue(t[i][pseudoDestinId]))+" ");
